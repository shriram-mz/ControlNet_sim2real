...
from transformers import AutoProcessor, BlipProcessor, Blip2ForConditionalGeneration
...
def prompts(img):
        ...
        #uploaded_file_dir = _launch_demo(model, tokenizer)
        # if img is not None: 
        ...
        inputs = processor(img, text="a realistic photo of", return_tensors="pt")
        ...
        if term1 in words:
            tokens.append("<indian-sidewalk>")
            response = " ".join(tokens)
        if term2 in words or term12 in words or term13 in words or term14 in words or term15 in words or term16 in words:
            tokens.append("<indian-person>")
            response = " ".join(tokens)
        if term3 in words or term11 in words: 
...
def process(input_image1, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta):
    with torch.no_grad():
        input_image = HWC3(input_image1)
...
with block:
    ...
    ips = [input_image, a_prompt, n_prompt, num_samples, image_resolution, detect_resolution, ddim_steps, guess_mode, strength, scale, seed, eta]
